            ...
            public async Task ClubInformation([Remainder]string clubName = null)
            {
                ...
                await Context.SendPaginatedConfirmAsync(0, (page) =>
                {
                    var embed = new EmbedBuilder()
                        .WithOkColor()
                        .WithTitle($"{club.Name}")
                        .WithDescription(GetText("club_level_x", Format.Bold(lvl.Level.ToString()), String.Format("{0:#,0}", club.Xp)))
            ...
            public Task ClubInvestLb(int page = 1)
            {
                ...
                return Context.SendPaginatedConfirmAsync(page, (curPage) =>
                {
                    ...
                    var embed = new EmbedBuilder()
                        ...
                        .WithOkColor()
                        .AddField(GetText("club_invests_members", curPage > 0 ? curPage * 10 : 1, club.Users.Count/((curPage + 1) * 10) < 1 ? club.Users.Count : (curPage + 1) * 10, club.Users.Count), Format.Bold(string.Join("\n", result.Skip((curPage) * 10).Take(10))), false)
